...
class GetMergedSongDetailUseCase(
  ...
  suspend operator fun invoke(userId: UserId, songId: SongId): Either<ReadError, MergedSongDetail> =
    txRunner.inRoTransaction {
      inCurrentTransaction(userId, songId)
    }

  suspend fun inCurrentTransaction(userId: UserId, songId: SongId): Either<ReadError, MergedSongDetail> {
    ...
    return if (hasOverrides) {
      ...
      val mergedSong = overrideReadRepository.getSongWithOverrides(userId, songId)
        ?: return ReadError.NotFound().left()
      ...
      val song = songReadRepository.get(songId) ?: return ReadError.NotFound().left()
