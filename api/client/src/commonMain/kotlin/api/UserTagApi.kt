 ...
 * Users can:
 * - View all tags (global + custom)
 * - Create custom tags
 * - Update/delete their custom tags
 * - Override global tag properties
...
interface UserTagApi {
  ...
  // Overrides for global tags
  suspend fun setOverride(id: TagIdDto, request: UserTagOverrideUpdateRequestDto): UserTagOverrideDto
  suspend fun resetOverride(id: TagIdDto): ResourceDeleteResult<TagIdDto>
}
...
internal class UserTagApiImpl(client: HttpClient) : BaseResourceApi(client), UserTagApi {
  ...
  override suspend fun update(id: TagIdDto, request: TagUpdateRequestDto): ResourceUpdateResult<TagIdDto> =
    executeUpdate<TagDetailDto, TagIdDto>(resourceId = id) {
      client.patch(UserTags.ById(id = id)) {
        header(HttpHeaders.ContentType, ContentType.Application.Json.toString())
        setBody(request)
      }
    }.getOrThrow()
  ...
  override suspend fun setOverride(id: TagIdDto, request: UserTagOverrideUpdateRequestDto): UserTagOverrideDto =
    execute<UserTagOverrideDto> {
      client.put(UserTags.Overrides.ById(parent = UserTags.Overrides(), id = id)) {
        header(HttpHeaders.ContentType, ContentType.Application.Json.toString())
        setBody(request)
      }
    }.getOrThrow()

  override suspend fun resetOverride(id: TagIdDto): ResourceDeleteResult<TagIdDto> =
    executeDelete<TagIdDto>(resourceId = id) {
      client.delete(UserTags.Overrides.ById(parent = UserTags.Overrides(), id = id))
    }.getOrThrow()
}
