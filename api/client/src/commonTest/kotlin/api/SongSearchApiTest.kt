 ...
 */
class SongSearchApiTest : StringSpec({
  ...
  "suggestions: should GET /v1/songs/search/suggestions and return parsed suggestions" {
  ...
  "suggestions: should pass query parameter" {
    val client = httpClientWith { req ->
  ...
  "suggestions: should pass bookId when provided" {
    val client = httpClientWith { req ->
  ...
  "suggestions: should pass limit when provided" {
    val client = httpClientWith { req ->
      req.url.parameters["query"] shouldBe "test"
  ...
  "suggestions: should not include optional params when null" {
    val client = httpClientWith { req ->
      req.url.parameters["query"] shouldBe "test"
  ...
  "suggestions: should return empty list when no suggestions" {
    val client = httpClientWith { req ->
      respond("[]", status = HttpStatusCode.OK, headers = headersOf("Content-Type", listOf(ContentType.Application.Json.toString())))
    }
    val api = SongApiImpl(client)
  ...
  "search: should GET /v1/songs/search and return parsed response" {
  ...
  "search: should pass all query parameters" {
  ...
  "search: should pass SearchType.LYRIC" {
    val response = SongSearchResponseDto(emptyList(), 0, false)
    val responseJson = json.encodeToString(response)
  ...
  "search: should not include optional params when null" {
    val response = SongSearchResponseDto(emptyList(), 0, false)
    val responseJson = json.encodeToString(response)
  ...
  "search: should return response with hasMore=true for paginated results" {
  ...
  "search: should return empty results when no matches" {
    val response = SongSearchResponseDto(emptyList(), 0, false)
    val responseJson = json.encodeToString(response)
